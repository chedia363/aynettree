...
$(function() {
    ...
    if($(".selectpicker").length > 0) {
     ...
     // init tinymce post
     if($(".js_wysiwyg_post").length > 0) {
        tinymce.init({
            selector: '.js_wysiwyg_post',
            menubar:'',
            language : 'ar',
            branding: false,
            elementpath : false,
            ...
            convert_urls : true,
            toolbar: ' bold italic | alignleft aligncenter alignright alignjustify | media |  forecolor  ltr rtl',
            plugins: [
                'placeholder advlist image  lists charmap  preview hr anchor pagebreak spellchecker',
...
// disable buttoon post tinymce
            ...
            if(response.callback) {
                ...
                if(type == "photos") {
                    ...
                    if(handle == "publisher") {
                        ...
                        var object = publisher.data(type);
                        object['source'] = response.file;
                        $("input[name='cvfile']").val(response.file);
        ...
        $.post(api['users/connect'], {'do': _do, 'id': id} , function(response) {
            if(response.callback) {
                ...
                if(_do == 'friend-accept') {
                    parent.append('<button type="button" class="btn btn-sm btn-success btn-delete js_friend-remove" data-uid="'+id+'"><i class="fa fa-check mr6"></i>'+__['Friends']+'</button>');
                }
            }
        }, "json")
        .fail(function() {
        ...
        $.post(api['users/connect'], {'do': _do, 'id': id} , function(response) {
            if(response.callback) {
                ...
                if(_do == 'friend-add') {
                    ...
                    _this.after('<button type"button" class="btn btn-sm btn-success js_friend-add" data-uid="'+id+'"><img class="joinuserr" src="'+window.location.origin+'/dev/content/themes/default/images/icons/interest.svg">'+__['Add Friend']+'</button>');
                }
                _this.remove();
            }
        }, "json")
    ...
    $('body').on('click', '.js_follow, .js_unfollow', function () {
        ...
        $.post(api['users/connect'], {'do': _do, 'id': id} , function(response) {
            if(response.callback) {
                ...
                if(_do == 'follow') {
                    _this.replaceWith('<button type="button" class="btn btn-sm btn-info js_unfollow" data-uid="'+id+'"><i class="fa fa-check mr6"></i>'+__['Following']+'</button>');
                } else {
                    _this.replaceWith('<button type="button" class="btn btn-sm btn-info js_follow" data-uid="'+id+'"><img class="followuserr" src="'+window.location.origin+'/dev/content/themes/default/images/icons/followbl.svg">'+__['Follow']+'</button>');
                }
            }
        }, "json")
        .fail(function() {
        ...
        $.post(api['users/connect'], {'do': _do, 'id': id, 'uid': uid} , function(response) {
            if(response.callback) {
                ...
                if(_this.hasClass('js_page-admin-addation')) {
                    ...
                    _this.replaceWith('<button type="button" class="btn btn-sm btn-primary js_page-admin-addation" data-id="'+id+'" data-uid="'+uid+'"><i class="fa fa-check mr6"></i>'+__['Make Admin']+'</button>');
                }
            }
        }, "json")
        .fail(function() {
        ...
        $.post(api['users/connect'], {'do': _do, 'id': id} , function(response) {
            if(response.callback) {
                ...
                if(_this.hasClass('js_join-group')) {
                    if(privacy == "public") {
                        _this.replaceWith('<button type="button" class="btn btn-sm btn-success btn-delete js_leave-group" data-id="'+id+'" data-privacy="'+privacy+'"><i class="fa fa-check mr7"></i>'+__['Joined']+'</button>');
                    } else {
                    ...
                    _this.replaceWith('<button type="button" class="btn btn-sm btn-success js_join-group" data-id="'+id+'" data-privacy="'+privacy+'"><img class="joinuserr" src="'+window.location.origin+'/dev/content/themes/default/images/icons/interest.svg">'+__['Join']+'</button>');
                }
            }
        }, "json")
        .fail(function() {
        ...
        $.post(api['users/connect'], {'do': _do, 'id': id, 'uid': uid} , function(response) {
            if(response.callback) {
                ...
                if(_this.hasClass('js_group-admin-addation')) {
                    ...
                    _this.replaceWith('<button type="button" class="btn btn-sm btn-primary js_group-admin-addation" data-id="'+id+'" data-uid="'+uid+'"><i class="fa fa-check mr6"></i>'+__['Make Admin']+'</button>');
                }
            }
        }, "json")
        .fail(function() {
    ...
    $('body').on('click', '.js_go-event, .js_ungo-event', function () {
        ...
        $.post(api['users/connect'], {'do': _do, 'id': id} , function(response) {
            if(response.callback) {
                ...
                if(_do == 'event-go') {
                    _this.replaceWith('<button type="button" class="btn btn-sm btn-success js_ungo-event" data-id="'+id+'"><i class="fa fa-check mr7"></i>'+__['Going']+'</button>');
                } else {
                    _this.replaceWith('<button type="button" class="btn btn-sm btn-success js_go-event" data-id="'+id+'"><img class="followuserr" src="'+window.location.origin+'/dev/content/themes/default/images/icons/all.png">'+__['Going']+'</button>');
                }
            }
        }, "json")
        .fail(function() {
        ...
        $.post(api['users/connect'], {'do': _do, 'id': id} , function(response) {
            if(response.callback) {
                ...
                if(_do == 'event-interest') {
                    _this.replaceWith('<button type="button" class="btn btn-sm btn-primary js_uninterest-event" data-id="'+id+'"><i class="fa fa-check mr6"></i>'+__['Interested']+'</button>');
                } else {
                    _this.replaceWith('<button type="button" class="btn btn-sm btn-primary js_interest-event" data-id="'+id+'"><img class="interestuserr" src="'+window.location.origin+'/dev/content/themes/default/images/icons/interestbl.svg">'+__['Interest']+'</button>');
                }
            }
        }, "json")
        .fail(function() {
    ...
    $('body').on('click', '.js_report', function (e) {
        ...
        var handle = $(this).data('handle');
if(handle=='post'){
        confirm(__['Report'], __['Are you sure you want to report this Post?'], function() {
        ...
        confirm(__['Report'], __['Are you sure you want to report this event?'], function() {
            $.post(api['data/report'], {'handle': handle, 'id': id}, function(response) {
                /* check the response */
                if(response.callback) {
                    eval(response.callback);
        ...
        confirm(__['Report'], __['Are you sure you want to report this Group?'], function() {
            $.post(api['data/report'], {'handle': handle, 'id': id}, function(response) {
                /* check the response */
                if(response.callback) {
                    eval(response.callback);
