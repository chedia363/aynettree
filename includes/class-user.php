...
class User {
    ...
    public function get_online_friends() {
        ...
        /* friends_ids by followings_ids to get followings onligne*/ 
        if(!$this->_data['followings_ids']) {
            return $online_friends;
        }
        /* make a list from viewer's friends */
        $friends_list = implode(',', $this->_data['followings_ids']);
        ...
        while($recipient = $get_recipients->fetch_assoc()) {
            ...
            if($system['chat_typing_enabled'] && $recipient['typing']) {
                ...
                if($get_recipient_status->fetch_assoc()['count'] == 0) {
                    ...
                    $conversation['typing_name_list'] .= $recipient['user_firstname'];
                }
            }
            ...
            if($system['chat_seen_enabled'] && $recipient['seen']) {
                ...
                $conversation['seen_name_list'] .= $recipient['user_firstname'];
            }
   ...
   public function get_stories() {
     ...
     * @return void
     */
    public function update_user_package($package_id, $package_name, $package_price, $package_verification) {
        global $db, $date;
        /* update user package */
        $verification_statement = ($package_verification)? "user_verified = '1',": ""; /* to not affect already verified users */
        $db->query(sprintf("UPDATE users SET ".$verification_statement." user_subscribed = '1', user_package = %s, user_subscription_date = %s, user_boosted_posts = '0', user_boosted_pages = '0' WHERE user_id = %s", secure($package_id, 'int'), secure($date), secure($this->_data['user_id'], 'int') )) or _error("SQL_ERROR_THROWEN");
        /* insert the payment */
