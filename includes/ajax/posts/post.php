...
if(isset($_POST['video'])) {
	...
	// }
	if(is_empty($_POST['video']->source)) {
...
if(isset($_POST['audio'])) {
	...
	if(is_empty($_POST['audio']->source)) {
		modal("MESSAGE", __("Message"), __("Please wait until finishing upload"));
	}
	// elseif(!is_object($_POST['audio'])) {

	// 	_error(400);

	// }
	// if(count($_POST['audio']) < 1) {
	// 	// _error(400);
	// 	modal("MESSAGE", __("Photos number should be between 1 and 12 photos"), __("The poll option you provided is too long. Please try again"));
	// }
...
if(isset($_POST['photos'])) {
	...
	if(count($photos) > 12) {
		// _error(400);
		modal("MESSAGE", __("Message"), __("Photos number should be between 1 and 12 photos"));
	}
	//end photos number do not exceed  12photos in the post
	// if(count($photos) > 12 || is_empty($photos)) {
	// 	// _error(400);
	// 	modal("MESSAGE", __("Message"), __("Please wait until finishing upload"));
	// }
	// if(is_empty($_POST['photos']->$photos[3])) {
	// 	modal("MESSAGE", __("Message"), __("Please wait until finishing upload"));
	// }
	// $src = ''.$filename;
